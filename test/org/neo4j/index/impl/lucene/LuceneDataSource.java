...
public class LuceneDataSource extends LogBackedXaDataSource
{
	...
	public static final Analyzer LOWER_CASE_WHITESPACE_ANALYZER =
			new Analyzer()
			{
				...
				{
					return new LowerCaseFilter(LUCENE_VERSION, new WhitespaceTokenizer(
							LUCENE_VERSION, reader));
				}
	 ...
	 */
	public LuceneDataSource(Config config, IndexStore indexStore,
			FileSystemAbstraction fileSystemAbstraction, XaFactory xaFactory)
	{
		super(DEFAULT_BRANCH_ID, DEFAULT_NAME);
		indexSearchers = new IndexSearcherLruCache(
				config.getInteger(Configuration.lucene_searcher_cache_size));
		indexWriters = new IndexWriterLruCache(
				config.getInteger(Configuration.lucene_writer_cache_size));
		caching = new Cache();
		...
		this.directoryGetter = config.getBoolean(Configuration.ephemeral) ? DirectoryGetter.MEMORY
				: DirectoryGetter.FS;
		...
		xaContainer = xaFactory.newXaContainer(this, this.baseStorePath + File.separator
				+ "lucene.log", cf, tf, null, null);
		...
		if (!isReadOnly)
		{
			...
			setKeepLogicalLogsIfSpecified(
					config.getBoolean(new GraphDatabaseSetting.BooleanSetting(
							"online_backup_enabled")) ? "true"
							: config.get(Configuration.keep_logical_logs), DEFAULT_NAME);
			...
			)
	{
		...
		synchronized (indexes)
		{
			...
			if (index == null)
			{
				index = new LuceneIndex.RelationshipIndex(luceneIndexImplementation, gdb,
						identifier);
				indexes.put(identifier, index);
			}
	...
	/**
	 * If nothing has changed underneath (since the searcher was last created or
	 * refreshed) {@code searcher} is returned. But if something has changed a
	 * refreshed searcher is returned. It makes use if the
	 * {@link IndexReader#openIfChanged(IndexReader, IndexWriter, boolean)}
	 * which faster than opening an index from scratch.
	 * 
	 * @param searcher
	 ...
	 */
	private Pair<IndexSearcherRef, AtomicBoolean> refreshSearcher(
			Pair<IndexSearcherRef, AtomicBoolean> searcher, IndexWriter writer)
	{
		try
		{
	...
	synchronized IndexSearcherRef getIndexSearcher(IndexIdentifier identifier, boolean incRef)
	{
		try
		{
			...
			if (searcher == null)
			{
				...
				searcher = Pair.of(new IndexSearcherRef(identifier, indexSearcher),
						new AtomicBoolean());
	...
	synchronized IndexWriter getIndexWriter(IndexIdentifier identifier)
	{
		...
		try
		{
			Directory dir = directoryGetter.getDirectory(baseStorePath, identifier); // getDirectory(
																						// baseStorePath,
																						// identifier
																						// );
			directoryExists(dir);
			...
			// added by robin, 2012-05-16
			System.out.println("Use robin's modification LuceneDataSource ...");
			writerConfig.setRAMBufferSizeMB(64.0D);
			TieredMergePolicy policy = new TieredMergePolicy();
			policy.setMaxMergeAtOnce(30);
	...
	private void makeSureAllIndexesAreInstantiated()
	{
		...
		for (String name : indexStore.getNames(Relationship.class))
		{
			...
			if (config.get(IndexManager.PROVIDER).equals(LuceneIndexImplementation.SERVICE_NAME))
			{
				getIndexWriter(new IndexIdentifier(LuceneCommand.RELATIONSHIP,
						relationshipEntityType, name));
			}
		}
	}
	...
	private static enum DirectoryGetter
	{
		...
		abstract Directory getDirectory(String baseStorePath, IndexIdentifier identifier)
				throws IOException;
	}
}
